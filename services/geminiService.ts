...
import { SoilData, Crop, FertilizerData, Coordinates, WeatherData, AlertsData, WeatherForecastDay, GeocodingResult, Language, CommandResponse } from '../types';
...
const wmoCodeToDescription: { [key: number]: string } = {
    0: 'Clear sky',
    1: 'Mainly clear',
    2: 'Partly cloudy',
    3: 'Overcast',
...
export const extractSoilDataFromImage = async (base64Image: string, mimeType: string): Promise<SoilData> => {
    ...
    return safelyParseJSON<SoilData>(response.text);
};

export const getCropRecommendations = async (soilData: SoilData, weatherData: WeatherData | null): Promise<Crop[]> => {
...
For each of the 3 recommended crops, provide the name, a detailed reason for its suitability based on the data, and an estimated yield potential in quintals per hectare. Return as a JSON array matching the schema.`;
    ...
    return safelyParseJSON<Crop[]>(response.text);
};

export const getFertilizerRecommendations = async (crop: Crop, soilData: SoilData): Promise<FertilizerData> => {
    ...
    return safelyParseJSON<FertilizerData>(response.text);
};


export const getLocationNameFromCoords = async (coords: Coordinates): Promise<string> => {
...
export const getCoordsFromLocationName = async (locationName: string): Promise<GeocodingResult | null> => {
...
export const getWeatherData = async (coords: Coordinates): Promise<WeatherData> => {
    ...
    try {
        ...
        const transformedData: WeatherData = {
            ...
            forecast: data.daily.time.slice(0, 7).map((dateString: string, index: number): WeatherForecastDay => {
...
export const getAlertsAndGuides = async (weatherData: WeatherData): Promise<AlertsData> => {
    ...
    return safelyParseJSON<AlertsData>(response.text);
}

export const interpretVoiceCommand = async (transcript: string): Promise<CommandResponse> => {
    ...
    return safelyParseJSON<CommandResponse>(response.text);
};

export const getFarmQueryResponse = async (query: string, language: Language): Promise<string> => {
