...
import { GeocodingResult, ChatMessage, Language, CommandResponse, Page, WeatherData, AlertsData, User } from './types';
import { 
...
import { SeedlingIcon, WindIcon, BotIcon, GovernmentIcon } from './components/icons';
...
const MainApp: React.FC<MainAppProps> = ({ user, onLogout }) => {
    ...
    const [alertsAndGuides, setAlertsAndGuides] = useState<AlertsData | null>(null);
    ...
    useEffect(() => {
        const loadInitialLocation = async () => {
            ...
            if (savedLocationJSON) {
                const savedLocation: GeocodingResult = JSON.parse(savedLocationJSON);
    ...
    const fetchWeatherForLocation = async (locationData: GeocodingResult) => {
        setLocationStatus('detecting');
    ...
    const searchForLocation = async (locationName: string) => {
    ...
    const speak = (text: string, lang: Language | 'en-US') => {
        try {
    ...
    const handleVoiceCommand = (command: CommandResponse) => {
