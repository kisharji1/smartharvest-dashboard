...
import { Scheme, SchemeBenefit, SchemeEligibility, SchemeIntervention, Language } from '../types';
import { ChevronLeftIcon, CheckCircleIcon, TargetIcon, BookOpenIcon, InfoIcon, AlertTriangleIcon } from './icons';
...
const renderComplexList = (items: (SchemeBenefit | SchemeEligibility | SchemeIntervention)[], lang: Language) => (
             ...
             if ('criteria' in item) { // SchemeEligibility
                ...
                return (
                    <li key={index}>
                        <strong>{title}</strong>
                        <ul className="list-circle list-outside ml-4 mt-1 space-y-1">
                            {criteria.map((c: string, i: number) => <li key={i}>{c}</li>)}
                        </ul>
                    </li>
                );
            }
...
export const SchemeDetailsPage: React.FC<SchemeDetailsPageProps> = ({ scheme, onBack, language }) => {
    const isTamil = language === 'ta-IN';
    
    // Helper to get translated titles
    const getTitle = (en: string, ta: string) => isTamil ? ta : en;
    ...
    return (
        <section className="bg-white p-4 md:p-6 rounded-lg shadow-lg">
            ...
            {scheme.benefits && <DetailSection title={getTitle('Benefits', 'பலன்கள்')} icon={<CheckCircleIcon className="w-6 h-6"/>}>{typeof scheme.benefits[0] === 'string' ? renderSimpleList(isTamil && scheme.benefits_ta ? scheme.benefits_ta as string[] : scheme.benefits as string[]) : renderComplexList(scheme.benefits as (SchemeBenefit | SchemeEligibility | SchemeIntervention)[], language)}</DetailSection>}
            ...
            {scheme.eligibility && <DetailSection title={getTitle('Eligibility', 'தகுதி')} icon={<CheckCircleIcon className="w-6 h-6"/>}>{typeof scheme.eligibility[0] === 'string' ? renderSimpleList(isTamil && scheme.eligibility_ta ? scheme.eligibility_ta as string[] : scheme.eligibility as string[]) : renderComplexList(scheme.eligibility as (SchemeBenefit | SchemeEligibility | SchemeIntervention)[], language)}</DetailSection>}
